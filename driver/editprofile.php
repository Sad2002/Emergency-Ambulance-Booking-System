...
$row = mysqli_fetch_assoc($res);
   ...
   if($oldusername!=$username){
    ...
    if(!isset($error)){ 
          if($file!= "")
          {
            $stmt = mysqli_query($conn,"SELECT image FROM  driver WHERE email='$email'");
...
mysqli_query($conn,"UPDATE users SET image='$new_image_name' WHERE email='$email'");
           ...
           $result = mysqli_query($conn,"UPDATE driver SET fname='$fname',lname='$lname',username='$username' WHERE email='$email'");
           if($result)
           {
       header("location:driveraccount.php?profile_updated=1");
           }
           else 
           {
